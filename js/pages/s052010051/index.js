...
import FacilityReg from "../s052010062";
import FacilityMngView from "./view/FacilityMngView";

const FacilityMng = observer(() => {
  ...
  const CheckedAllHandler = ({ target }) => {
    ...
    if (target.checked) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        document.getElementById(`check${list[i].slaveId}`).checked = true
      }
    } else {
      setCheckList([]);
      for (let i = 0; i < list.length; i++) {
        // document.getElementById(`${i}check`).checked = false
        document.getElementById(`check${list[i].slaveId}`).checked = false
      }
    }
  }
  ...
  const initPage = async () => {
    try {
      ...
      const { data } = await API_GET(`/s052010050/emSlave`, params)
      setList(data?.data || [])
  ...
  const onClickSearch = async (curPage) => {
    try {
      const { data } = await API_GET(`/s052010050/emSlave?curPage=${curPage}`, searchParams)
      setList(data?.data || [])
      setCheckList([]);
  ...
  useEffect(()=> {
    ...
    ws.current.onopen = () => {
      console.log("FacilityMng WebSocket Connected");
    };
    ws.current.onclose = () => {
      console.log("FacilityMng WebSocket Disconnected");
    };
  ...
  return useObserver(() => (
    <div>
      ...
      />
      <ModalPortal>
        <FacilityReg closeModal={closeModal} checkedSlave={checkList[0]} onClickSearch={onClickSearch} initPage={initPage}/>
      </ModalPortal>
    </div>
  ));
});
